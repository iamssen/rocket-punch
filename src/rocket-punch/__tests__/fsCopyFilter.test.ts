...
describe('fsPackagesCopyFilter()', () => {
  test.each([
    ...
    expect(fsCopyFilter(path.join('/Users/foo/bar', pattern))).toBeFalsy();
    expect(fsCopyFilter(path.win32.join('C:/foo/bar', pattern))).toBeFalsy();
  });
  ...
  test.each([
    ...
    expect(fsCopyFilter(path.join('/Users/foo/bar', pattern))).toBeTruthy();
    expect(fsCopyFilter(path.win32.join('C:/foo/bar', pattern))).toBeTruthy();
  });
});
